# RunPod Serverless "swap-only" endpoint using InsightFace inswapper_128 (CPU-safe)
# Input:
# {
...
# }
# Output: { "status": "success", "image_b64": "<base64>" } or { "status": "error", "message": "..." }
...
# -------- model loaders (download on first run if needed) --------
def _load_detector():
    os.makedirs(INSIGHTFACE_HOME, exist_ok=True)
    app = FaceAnalysis(name=DET_NAME, root=INSIGHTFACE_HOME)
    # ctx_id=0 tries GPU if available; otherwise CPU. Safe for serverless CPU.
    app.prepare(ctx_id=0, det_size=(DET_SIZE, DET_SIZE))
    return app

def _load_swapper():
    # prefer local file if present; otherwise download on first run
    local_path = os.path.join(INSIGHTFACE_HOME, "models", "inswapper_128.onnx")
    if os.path.exists(local_path):
        return get_model(local_path, download=False)
    return get_model("inswapper_128.onnx", download=True, download_zip=True)

# Cold-start load (will download once on very first request if cache empty)
app = _load_detector()
